...
class fourteen_mm_vhm_drill_and_reibahle:
    def __init__(self, new_cad_model):
        ...
        self.radius = 8.375
        self.depth = np.random.uniform(25, 31)
    ...
    def _build_vectors(self):
        return {
            "direction_1": {
                ...
                "vector_B": mdc.dvec3(-self.radius, 0, -0.0001),
                "vector_C": mdc.dvec3(-self.radius, 0, self.depth),
                "vector_D": mdc.dvec3(0, 0, self.total_depth),
            },
            "direction_2": {
                "vector_A": mdc.dvec3(0, 0, self.new_cad_model.height - self.total_depth),
                "vector_B": mdc.dvec3(-self.radius, 0, self.new_cad_model.height - self.depth),
                "vector_C": mdc.dvec3(-self.radius, 0, self.new_cad_model.height),
                "vector_D": mdc.dvec3(0, 0, self.new_cad_model.height),
            },
            "direction_3": {
                "vector_A": mdc.dvec3(0, -0.0001, 0),
                "vector_B": mdc.dvec3(-self.radius, -0.0001, 0),
                "vector_C": mdc.dvec3(-self.radius, self.depth, 0),
                "vector_D": mdc.dvec3(0, self.total_depth, 0),
            },
            "direction_4": {
                "vector_A": mdc.dvec3(0, self.new_cad_model.depth - self.total_depth, 0),
                "vector_B": mdc.dvec3(-self.radius, self.new_cad_model.depth - self.depth, 0),
                "vector_C": mdc.dvec3(-self.radius, self.new_cad_model.depth, 0),
                "vector_D": mdc.dvec3(0, self.new_cad_model.depth, 0),
            },
            "direction_5": {
                "vector_A": mdc.dvec3(-0.0001, 0, 0),
                "vector_B": mdc.dvec3(-0.0001, 0, -self.radius),
                "vector_C": mdc.dvec3(self.depth, 0, -self.radius),
                "vector_D": mdc.dvec3(self.total_depth, 0, 0),
            },
            "direction_6": {
                "vector_A": mdc.dvec3(self.new_cad_model.length - self.total_depth, 0, 0),
                "vector_B": mdc.dvec3(self.new_cad_model.length - self.depth, 0, -self.radius),
                "vector_C": mdc.dvec3(self.new_cad_model.length, 0, -self.radius),
